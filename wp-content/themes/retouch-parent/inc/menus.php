...
if ( ! function_exists( 'fw_theme_select_menu_message' ) ) :
    ...
    {
        echo '<nav id="nav"><p style="color:#ffffff;">'.__('Please go to the','fw').' <a href="' . admin_url('nav-menus.php') . '" target="_blank">'.__('Menu','fw').'</a> '.__('section, create a  menu and then select the newly created menu from the Theme Locations box from the left.','fw').'</p></nav>';
    }
endif;